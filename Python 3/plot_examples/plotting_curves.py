# Plotting curves with matplotlib pyplot. Python ver. 2.7
...
def second_pol_plot(A, B, C, start, stop, step=1):
    ...
    plt.plot(x_axe, y_axe)  # Plots the curve using the values in x_axe and y_axe.

    # We check to see which solution we have to plot

    if len(single_point) != 0:  # Testing for occurance of a single root.
        plt.plot(single_point[0][0], 0, 'ro')   # Printing a red dot at the root point.
    elif len(double_point) != 0:    # Testing for occurance of a double root.        
        plt.plot(double_point[0][0], 0,'ro')    # Printing a red dot at the root point.
        plt.plot(double_point[1][0], 0,'ro')    # Printing a red dot at the root point.

    plt.axis([x_min, x_max, y_min, y_max])  # Sets the min and max values for the x and y axis.
    plt.axhline(0, 0, color = 'r')  # Draws the x axe.
    plt.axvline(0, 0, color = 'r')  # Draws the y axe.
    plt.annotate('y-axis', xy = (0.25, y_max - 1), xytext = (0.25, y_max - 1)) # Place the 'y-axis' on the plot.
    plt.annotate('x-axis', xy = (x_max - 1, - 1), xytext = (x_max - 1, - 1))    # Place the 'x-axis' on the plot.
